...
import space.maxkonkin.nasapicbot.util.NasaUtil.cloneWithReplacedUrl
import java.io.IOException

@Component
class NasaApiClient(private val nasaAPIConfig: NasaAPIConfig) {
    ...
    fun getNASAObject(uri: String): NasaTo {
        HttpClients.createDefault().use { client ->
            client.execute(HttpGet(uri)).use { response ->
                val input = mapper.readValue(response.entity.content, NasaTo::class.java)
                return if (input.mediaType == "video" && input.url!!.contains("embed/")) {
    ...
    private fun getFiltered(tos: List<NasaTo>): List<NasaTo> {
        ...
        for (to in tos) {
            if (to.mediaType == "video" && to.url!!.contains("embed/")) {
